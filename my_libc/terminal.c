...
void terminal_put_char(struct terminal* terminal, char c) {
    if (c == '\n') {
        ...
        terminal_put_entry_at(terminal, c, terminal->color, terminal->column, terminal->row);
        ++terminal->column;
...
void terminal_scroll(struct terminal* terminal, long amount) {
    ...
    if (amount > 0) {
        for (long i = amount; i < VGA_HEIGHT; ++i) {
            memory_copy(
                &terminal->buffer[(i - amount) * VGA_WIDTH],
                &terminal->buffer[i * VGA_WIDTH],
                VGA_WIDTH * sizeof(uint16_t)
            );
        }
        for (long i = VGA_HEIGHT - amount; i < VGA_HEIGHT; ++i) {
            memory_set(&terminal->buffer[i * VGA_WIDTH], 0, VGA_WIDTH * sizeof(uint16_t));
        }
    } else {
